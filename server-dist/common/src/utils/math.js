...
exports.EaseFunctions = exports.Collision = exports.Statistics = exports.Geometry = exports.Numeric = exports.Angle = exports.TAU = exports.HALF_PI = exports.PI = exports.τ = exports.halfπ = exports.π = void 0;
exports.calculateDoorHitboxes = calculateDoorHitboxes;
exports.resolveStairInteraction = resolveStairInteraction;
const hitbox_1 = require("./hitbox");
const vector_1 = require("./vector");
exports.π = Math.PI;
exports.halfπ = exports.π / 2;
exports.τ = 2 * exports.π;
// For accessibility
exports.PI = exports.π;
exports.HALF_PI = exports.π / 2;
exports.TAU = 2 * exports.π;
exports.Angle = Object.freeze({
    /**
     ...
     */
    normalize(radians) {
        return exports.Numeric.absMod(radians - exports.π, exports.τ) - exports.π;
    },
    /**
     ...
     */
    minimize(start, end) {
        return exports.Numeric.absMod(end - start + exports.π, exports.τ) - exports.π;
    },
    /**
    ...
    radiansToDegrees(radians) {
        return radians / exports.π * 180;
    },
    orientationToRotation(orientation) {
        return -this.normalize(orientation * exports.halfπ);
    },
    /**
...
exports.Numeric = Object.freeze({
    ...
    delerp(t, a, b) {
        return exports.Numeric.clamp((t - a) / (b - a), 0.0, 1.0);
    },
    /**
     ...
     */
    remap(value, min0, max0, min1, max1) {
        return exports.Numeric.lerp(min1, max1, exports.Numeric.clamp((value - min0) / (max0 - min0), 0, 1));
    },
    /**
...
exports.Geometry = Object.freeze({
    /**
    ...
    transformRectangle(pos, min, max, scale, orientation) {
        min = vector_1.Vec.scale(min, scale);
        max = vector_1.Vec.scale(max, scale);
        if (orientation !== 0) {
            const minX = min.x;
            const minY = min.y;
            const maxX = max.x;
            ...
            switch (orientation) {
                case 1:
                    min = (0, vector_1.Vec)(maxX, minY);
                    max = (0, vector_1.Vec)(minX, maxY);
                    break;
                case 2:
                    min = (0, vector_1.Vec)(maxX, maxY);
                    max = (0, vector_1.Vec)(minX, minY);
                    break;
                case 3:
                    min = (0, vector_1.Vec)(minX, maxY);
                    max = (0, vector_1.Vec)(maxX, minY);
                    break;
            }
        }
        return {
            min: vector_1.Vec.addAdjust(pos, min, orientation),
            max: vector_1.Vec.addAdjust(pos, max, orientation)
        };
    }
});
exports.Statistics = Object.freeze({
    average(values) {
    ...
    stddev(values) {
        const avg = exports.Statistics.average(values);
        return Math.sqrt(exports.Statistics.average(values.map(v => (v - avg) ** 2)));
    },
    median(values) {
        if (values.length % 2) {
...
exports.Collision = Object.freeze({
    /**
    ...
    rectangleCollision(min, max, pos, rad) {
        const cpt = {
            x: exports.Numeric.clamp(pos.x, min.x, max.x),
            y: exports.Numeric.clamp(pos.y, min.y, max.y)
        };
    ...
    distanceBetweenRectangleCircle(min, max, circlePos, circleRad) {
        const distX = exports.Numeric.clamp(circlePos.x, min.x, max.x) - circlePos.x;
        const distY = exports.Numeric.clamp(circlePos.y, min.y, max.y) - circlePos.y;
    ...
    distanceBetweenRectangles(min1, max1, min2, max2) {
        const distX = exports.Numeric.max(min1.x, Math.min(max1.x, min2.x, max2.x)) - exports.Numeric.min(min1.x, Math.max(max1.x, min2.x, max2.x));
        const distY = exports.Numeric.max(min1.y, Math.min(max1.y, min2.y, max2.y)) - exports.Numeric.min(min1.y, Math.max(max1.y, min2.y, max2.y));
    ...
    lineIntersectsLine(startA, endA, startB, endB) {
        const x1 = exports.Geometry.signedArea(startA, endA, endB);
        const x2 = exports.Geometry.signedArea(startA, endA, startB);
        if (x1 !== 0 && x2 !== 0 && x1 * x2 < 0) {
            const x3 = exports.Geometry.signedArea(startB, endB, startA);
            const x4 = x3 + x2 - x1;
            if (x3 * x4 < 0) {
                return vector_1.Vec.add(startA, vector_1.Vec.scale(vector_1.Vec.sub(endA, startA), x3 / (x3 - x4)));
            }
        }
        return null;
    },
     ...
     * not exist.
     */
    lineSegmentIntersection(segmentAStart, segmentAEnd, segmentBStart, segmentBEnd) {
        ...
        // line segments.
        const Sa = vector_1.Vec.sub(segmentAEnd, segmentAStart);
        const Sb = vector_1.Vec.sub(segmentBEnd, segmentBStart);
        ...
        const R = vector_1.Vec.sub(segmentAStart, segmentBStart);
        ...
        if ((0 <= ta && ta <= 1) && (0 <= tb && tb <= 1)) {
            return (0, vector_1.Vec)(segmentAStart.x + (ta * Sa.x), segmentAStart.y + (ta * Sa.y));
        }
    ...
    lineIntersectsCircle(s0, s1, pos, rad) {
        let d = vector_1.Vec.sub(s1, s0);
        const len = exports.Numeric.max(vector_1.Vec.len(d), 0.000001);
        d = vector_1.Vec.normalizeSafe(d);
        const m = vector_1.Vec.sub(s0, pos);
        const b = vector_1.Vec.dotProduct(m, d);
        const c = vector_1.Vec.dotProduct(m, m) - rad * rad;
        if (c > 0 && b > 0)
            return null;
        const discSq = b * b - c;
        if (discSq < 0)
        ...
        const t = -b < disc
            ...
            : -b - disc;
        if (t <= len) {
            const point = vector_1.Vec.add(s0, vector_1.Vec.scale(d, t));
            return {
                point,
                normal: vector_1.Vec.normalize(vector_1.Vec.sub(point, pos))
            };
        }
        return null;
    },
    ...
    lineIntersectsRect(s0, s1, min, max) {
        ...
        if (absDx > eps) {
            ...
            tmin = exports.Numeric.max(tmin, exports.Numeric.min(tx1, tx2));
            tmax = exports.Numeric.min(tmax, exports.Numeric.max(tx1, tx2));
            if (tmin > tmax)
                return null;
        }
        if (absDy > eps) {
            ...
            tmin = exports.Numeric.max(tmin, exports.Numeric.min(ty1, ty2));
            tmax = exports.Numeric.min(tmax, exports.Numeric.max(ty1, ty2));
            if (tmin > tmax)
                return null;
        }
        if (tmin > dist)
            return null;
        // Hit
        const p = vector_1.Vec.add(s0, vector_1.Vec.scale(d, tmin));
        // Intersection normal
        const c = vector_1.Vec.add(min, vector_1.Vec.scale(vector_1.Vec.sub(max, min), 0.5));
        const p0 = vector_1.Vec.sub(p, c);
        const d0 = vector_1.Vec.scale(vector_1.Vec.sub(min, max), 0.5);
        const x = p0.x / Math.abs(d0.x) * 1.001;
        const y = p0.y / Math.abs(d0.y) * 1.001;
        return {
            point: p,
            normal: vector_1.Vec.normalizeSafe((0, vector_1.Vec)(Math.trunc(x), Math.trunc(y)), (0, vector_1.Vec)(1, 0))
        };
    },
    /**
    ...
    circleCircleIntersection(centerA, radiusA, centerB, radiusB) {
        ...
        const toP1 = vector_1.Vec.sub(centerB, centerA);
        const distSqr = vector_1.Vec.squaredLen(toP1);
        return distSqr < r * r
            ? {
                dir: vector_1.Vec.normalizeSafe(toP1),
    ...
    rectCircleIntersection(min, max, pos, radius) {
        if (min.x <= pos.x && pos.x <= max.x
            ...
            const halfDimension = vector_1.Vec.scale(vector_1.Vec.sub(max, min), 0.5);
            const p = vector_1.Vec.sub(pos, vector_1.Vec.add(min, halfDimension));
            const xp = Math.abs(p.x) - halfDimension.x - radius;
            const yp = Math.abs(p.y) - halfDimension.y - radius;
            return xp > yp
                ? {
                    dir: (0, vector_1.Vec)(p.x > 0 ? -1 : 1, 0),
                    pen: -xp
                }
                : {
                    dir: (0, vector_1.Vec)(0, p.y > 0 ? -1 : 1),
                    pen: -yp
                };
        }
        const dir = vector_1.Vec.sub((0, vector_1.Vec)(exports.Numeric.clamp(pos.x, min.x, max.x), exports.Numeric.clamp(pos.y, min.y, max.y)), pos);
        const dstSqr = vector_1.Vec.squaredLen(dir);
        if (dstSqr < radius * radius) {
            const dst = Math.sqrt(dstSqr);
            return {
                dir: vector_1.Vec.normalizeSafe(dir),
                pen: radius - dst
            };
        }
        return null;
    ...
    distanceToLine(p, a, b) {
        const ab = vector_1.Vec.sub(b, a);
        return vector_1.Vec.squaredLen(vector_1.Vec.sub(vector_1.Vec.add(a, vector_1.Vec.scale(ab, exports.Numeric.clamp(vector_1.Vec.dotProduct(vector_1.Vec.sub(p, a), ab) / vector_1.Vec.dotProduct(ab, ab), 0, 1))), p));
    },
    distToSegmentSq(p, a, b) {
        const ab = vector_1.Vec.sub(b, a);
        const c = vector_1.Vec.dotProduct(vector_1.Vec.sub(p, a), ab) / vector_1.Vec.dotProduct(ab, ab);
        const d = vector_1.Vec.add(a, vector_1.Vec.scale(ab, exports.Numeric.clamp(c, 0.0, 1.0)));
        const e = vector_1.Vec.sub(d, p);
        return vector_1.Vec.dotProduct(e, e);
    },
    distToPolygonSq(p, poly) {
        let closestDistSq = Number.MAX_VALUE;
        for (let i = 0; i < poly.length; i++) {
            const a = poly[i];
            const b = i === poly.length - 1 ? poly[0] : poly[i + 1];
            const distSq = exports.Collision.distToSegmentSq(p, a, b);
    ...
    distToPolygon(p, poly) {
        return Math.sqrt(exports.Collision.distToPolygonSq(p, poly));
    },
    /**
     * Source
    ...
    rayIntersectsLine(origin, direction, lineA, lineB) {
        const segment = vector_1.Vec.sub(lineB, lineA);
        const segmentPerp = (0, vector_1.Vec)(segment.y, -segment.x);
        const perpDotDir = vector_1.Vec.dotProduct(direction, segmentPerp);
        // If lines are parallel, no intersection
        if (Math.abs(perpDotDir) <= 1e-7)
            return null;
        const d = vector_1.Vec.sub(lineA, origin);
        const distanceAlongRay = vector_1.Vec.dotProduct(segmentPerp, d) / perpDotDir;
        const distanceAlongLine = vector_1.Vec.dotProduct((0, vector_1.Vec)(direction.y, -direction.x), d) / perpDotDir;
    ...
    rayIntersectsPolygon(origin, direction, polygon) {
        ...
        for (let i = 0, length = polygon.length, j = length - 1; i < length; j = i++) {
            const dist = exports.Collision.rayIntersectsLine(origin, direction, polygon[j], polygon[i]);
            if (dist !== null && dist < t) {
                intersected = true;
                t = dist;
            }
    ...
    rectRectIntersection(min0, max0, min1, max1) {
        const e0 = vector_1.Vec.scale(vector_1.Vec.sub(max0, min0), 0.5);
        const e1 = vector_1.Vec.scale(vector_1.Vec.sub(max1, min1), 0.5);
        const n = vector_1.Vec.sub(vector_1.Vec.add(min1, e1), vector_1.Vec.add(min0, e0));
        const xo = e0.x + e1.x - Math.abs(n.x);
        const yo = e0.y + e1.y - Math.abs(n.y);
        return xo > 0 && yo > 0
            ? xo > yo
                ? {
                    dir: (0, vector_1.Vec)(Math.sign(n.x) || 1, 0),
                    pen: xo
                }
                : {
                    dir: (0, vector_1.Vec)(0, Math.sign(n.y) || 1),
                    pen: yo
                }
            : null;
    }
...
function calculateDoorHitboxes(definition, position, rotation) {
    if (!(definition.hitbox instanceof hitbox_1.RectangleHitbox) || !definition.isDoor) {
    ...
    switch (definition.operationStyle) {
        case "slide": {
            const openHitbox = exports.Geometry.transformRectangle(vector_1.Vec.addAdjust(position, (0, vector_1.Vec)((definition.hitbox.min.x - definition.hitbox.max.x) * (definition.slideFactor ?? 1), 0), rotation), definition.hitbox.min, definition.hitbox.max, 1, rotation);
            return {
                openHitbox: new hitbox_1.RectangleHitbox(openHitbox.min, openHitbox.max)
            };
        }
        case "swivel":
        default: {
            const openRectangle = exports.Geometry.transformRectangle(vector_1.Vec.addAdjust(position, vector_1.Vec.add(definition.hingeOffset, (0, vector_1.Vec)(-definition.hingeOffset.y, definition.hingeOffset.x)), rotation), definition.hitbox.min, definition.hitbox.max, 1, exports.Numeric.absMod(rotation + 1, 4));
            const openAltRectangle = exports.Geometry.transformRectangle(vector_1.Vec.addAdjust(position, vector_1.Vec.add(definition.hingeOffset, (0, vector_1.Vec)(definition.hingeOffset.y, -definition.hingeOffset.x)), rotation), definition.hitbox.min, definition.hitbox.max, 1, exports.Numeric.absMod(rotation - 1, 4));
            return {
                openHitbox: new hitbox_1.RectangleHitbox(openRectangle.min, openRectangle.max),
                openAltHitbox: new hitbox_1.RectangleHitbox(openAltRectangle.min, openAltRectangle.max)
            };
        }
    }
}
...
*/
function resolveStairInteraction(definition, obstacleOrientation, obstacleHitbox, obstacleLayer, targetPosition) {
    ...
    const [high, low] = [
        exports.Numeric.absMod(highDef - obstacleOrientation, 4),
        exports.Numeric.absMod(lowDef - obstacleOrientation, 4)
    ];
    /*
   ...
   */
    ...
    */
    ...
    if (isBottomRightToTopLeft) {
        ...
        if (objIsRight // trivial rhs check
        ...
        if (objIsLeft // trivial lhs check
 ...
 */
exports.EaseFunctions = Object.freeze({
    linear: (t) => t,
    sineIn: (t) => 1 - Math.cos(t * exports.halfπ),
    sineOut: (t) => Math.sin(t * exports.halfπ),
    sineInOut: (t) => (1 - Math.cos(exports.π * t)) / 2,
    ...
    elasticIn: (t) => t === 0 || t === 1
        ? t
        : -(2 ** (10 * (t - 1))) * Math.sin(exports.π * (40 * (t - 1) - 3) / 6),
    elasticOut: (t) => t === 0 || t === 1
        ? t
        : 2 ** (-10 * t) * Math.sin(exports.π * (40 * t - 3) / 6) + 1,
    elasticInOut: (t) => t === 0 || t === 1
        ? t
        : t < 0.5
            ? -(2 ** (10 * (2 * t - 1) - 1)) * Math.sin(exports.π * (80 * (2 * t - 1) - 9) / 18)
            : 2 ** (-10 * (2 * t - 1) - 1) * Math.sin(exports.π * (80 * (2 * t - 1) - 9) / 18) + 1,
    elasticOut2: (t) => (Math.pow(2, t * -10) * Math.sin(((t - 0.75 / 4) * (exports.π * 2)) / 0.75) + 1),
