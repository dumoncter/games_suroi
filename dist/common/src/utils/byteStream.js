...
class ByteStream {
     ...
     */
    writeRotation(value) {
        return this.writeUint8((value / math_1.τ + 0.5) * 255 + 0.5);
    }
    /**
     ...
     */
    readRotation() {
        return math_1.τ * (this.readUint8() / 255 - 0.5);
    }
    /**
     * Writes an angle in radians over one byte, with the angle falling inside `[-π, π]`
     * @param value The angle to write. Undefined behavior occurs if this value is not inside `[-π, π]`
     ...
     */
    writeRotation2(value) {
        return this.writeUint16((value / math_1.τ + 0.5) * 65535 + 0.5);
    }
    /**
     * Reads an angle in radians over two bytes, with the angle falling inside `[-π, π]`
     * @returns A float guaranteed to be finite, not `NaN`, and falling within `[-π, π]`
     ...
     */
    readRotation2() {
        return math_1.τ * (this.readUint16() / 65535 - 0.5);
    }
    /**
    ...
    b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, bA, bB, bC, bD, bE, bF) {
        return this.writeUint16((b0 ? 1 : 0)
            + (b1 ? 2 : 0)
            + (b2 ? 4 : 0)
            + (b3 ? 8 : 0)
            + (b4 ? 16 : 0)
     ...
     */
    writeArray(source, elementWriter, bytes = 1) {
        const length = math_1.Numeric.min(source.length, bytes === 1 ? 255 : 65535);
        if (bytes === 1) {
     ...
     */
    writeSet(source, elementWriter, bytes = 1) {
        const length = math_1.Numeric.min(source.size, bytes === 1 ? 255 : 65535);
        if (bytes === 1) {
            this.writeUint8(length);
        }
        else if (bytes === 2) {
     ...
     */
    readArray(elementReader, bytes = 1) {
        let length;
        if (bytes === 1) {
     ...
     */
    writeStream(src, offset = 0, length = src.index - offset) {
